...
test.describe("Homepage Navigation", () => {
  ...
  test("should display hero section with main headline", async ({ page }) => {
    ...
    await expect(
      page.locator("text=/We Build Intelligent Autonomous Systems/i"),
    ).toBeVisible();
  });
...
test.describe("Division Navigation", () => {
  test("should navigate back to homepage from division page", async ({
    page,
  }) => {
    await page.goto("/divisions/qr-ai");
...
test.describe("404 Error Handling", () => {
  test("should handle non-existent division page gracefully", async ({
    page,
  }) => {
